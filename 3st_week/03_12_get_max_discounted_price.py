#Q.
# 다음과 같이 숫자로 이루어진 배열이 두 개가 있다.
# 하나는 상품의 가격을 담은 배열이고, 하나는 쿠폰을 담은 배열이다.
# 쿠폰의 할인율에 따라 상품의 가격을 할인 받을 수 있다.
# 이 때, 최대한 할인을 많이 받는다면 얼마를 내야 하는가?
# 단, 할인쿠폰은 한 제품에 한 번씩만 적용 가능하다.
shop_prices = [30000, 2000, 1500000]
user_coupons = [20, 40]

# [내풀이]
def get_max_discounted_price(prices, coupons):
    # shop_prices 의 가장 큰 값이랑
    # user_coupons의 가장 큰 값이랑 묶어서 할인된 값을 구하고
    # 그 다음으로 가장 큰 값이랑 묶어서 할인된 값을 구한다.
    # coupons을 기준으로 for문을 돌려야 한다.
    # prices를 정렬한 뒤 stack에 저장한다.
    total_price = 0         # 총 금액
    discounted_price = 0    # 할인 금액

    prices.sort(reverse=True)   # 내림차순 정렬
    coupons.sort()              # 오름차순 정렬 -> stack 사용
    for price in prices:
        total_price += price
    for price in prices:
        if not coupons:    # stack_coupon이 존재하지 않는다면 break
            break
        discounted_price += price * (coupons.pop() / 100)
    return round(total_price - discounted_price)


# [강사풀이]
def get_max_discounted_price_1(prices, coupons):
    prices.sort(reverse=True)   # 내림차순 정렬
    coupons.sort(reverse=True)  # 내침차순 정렬

    price_index = 0
    coupon_index = 0
    max_discounted_price = 0
    while price_index < len(prices) and coupon_index < len(coupons): # 현재 가격과 쿠폰이 모두 배열 내의 원소일 때
        discount_price = prices[price_index] * (100 - coupons[coupon_index]) / 100
        max_discounted_price += discount_price
        price_index += 1
        coupon_index += 1
    while price_index < len(prices):
        max_discounted_price += prices[price_index]
        price_index += 1

    return max_discounted_price


print("정답 = 926000 / 현재 풀이 값 = ", get_max_discounted_price([30000, 2000, 1500000], [20, 40]))
print("정답 = 485000 / 현재 풀이 값 = ", get_max_discounted_price([50000, 1500000], [10, 70, 30, 20]))
print("정답 = 1550000 / 현재 풀이 값 = ", get_max_discounted_price([50000, 1500000], []))
print("정답 = 1458000 / 현재 풀이 값 = ", get_max_discounted_price([20000, 100000, 1500000], [10, 10, 10]))